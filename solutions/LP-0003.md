...
- [x] **E2E integration tests run against a LEZ sequencer (standalone mode), in
      ...
      Across the 18 green runs in the workflow history it takes **27 to 126
      minutes, median 73** — the spread is the runner it lands on, not the work,
      which is identical every time. The reviewed commit has **two** of them,
      because the push and the `v1.0.0` tag each triggered one:
      [53 minutes](https://github.com/edenbd1/lp-0003-private-airdrop-distributor/actions/runs/32675568443)
      and [2 h 05](https://github.com/edenbd1/lp-0003-private-airdrop-distributor/actions/runs/32675681910)
      — the same lifecycle proved from nothing twice, on different runners, with
      the same result.

      `docs/criteria-checklist.md` at the reviewed commit quotes a narrower
      58–80 minutes. That was measured over fewer runs; the figure above is
      recomputed from the workflow history, which anyone can open.
